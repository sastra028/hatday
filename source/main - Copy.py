...
def scan_all_soils_by_color(min_area=800, max_area=15000):
    """สแกนหาแปลงดินว่างจากช่วงสี HSV (ขยายช่วงสีให้กว้างขึ้น)"""
    check_and_close_popups()
    ...
    # 🎨 ขยายช่วงสีน้ำตาล/ส้ม/เหลืองดิน ให้ครอบคลุมมากขึ้น
    ...
    for cnt in contours:
        ...
        if min_area <= area <= max_area:
            ...
            # NMS กันจุดซ้ำใกล้กันเกินไป
            if not any(
                np.hypot(center_x - px, center_y - py) < 20
                for px, py in found_points
            ):
                found_points.append((center_x, center_y))
                sample_height, sample_width = h, w

    if not found_points:
        print("❌ ไม่พบแปลงดินว่างด้วย Color Detection")
        return [], 0, 0

    # เรียงลำดับจาก ซ้ายบน -> ขวาล่าง
    found_points.sort(key=lambda p: (p[1] // 25, p[0]))
    print(
        f"📍 สแกนพบแปลงดินว่างทั้งหมด {len(found_points)} แปลง (ด้วย Color Contour)"
    )
    return found_points, sample_width, sample_height
...
def main_automation_loop():
    ...
    # 📌 ข้อ 1: เรียกใช้ scan_all_soils_by_color สแกนและจำตำแหน่งทุกแปลงก่อนเริ่ม Process
    soils, s_width, s_height = scan_all_soils_by_color()
    ...
    print(f"📌 บันทึกสำเร็จ {len(soils)} แปลง | แปลงซ้ายบนสุดคือ: {top_left_soil}")

    # 🔄 เข้าสู่ Process ทำงานวนลูป (ข้อ 2 - 9)
    while True:
