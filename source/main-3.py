...
def scan_all_soils_by_color(min_area=1500, max_area=8000):
    """สแกนหาแปลงดินว่างจากช่วงสี HSV และ Contour รูปทรงแปลงผัก"""
    check_and_close_popups()
    ...
    # ช่วงสีน้ำตาล/ส้มของแปลงดินในเกม Hay Day
    lower_brown = np.array([8, 100, 100])
    upper_brown = np.array([22, 255, 240])
    ...
    found_points.sort(key=lambda p: (p[1] // 25, p[0]))
    print(f"📍 สแกนพบแปลงดินว่างทั้งหมด {len(found_points)} แปลง (ด้วย Color Contour)")
