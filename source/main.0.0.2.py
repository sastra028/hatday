...
# --- ⚙️ ตั้งค่าชื่อไฟล์รูปภาพแม่แบบ ---
# รายชื่อแม่แบบพืชสุก (สแกนหาตัวใดตัวหนึ่งในนี้)
...
def apply_ui_mask_color(screenshot_bgr):
    """ถมสีดำ (0, 0, 0) บนภาพสี เพื่อปิด UI รอบนอกอย่างเด็ดขาด"""
    h_scr, w_scr, _ = screenshot_bgr.shape

    # 🚫 1. มุมขวาบน (เหรียญ/เพชร/เตาอบ/โปรโมชัน)
    cv2.rectangle(
        screenshot_bgr,
        (int(w_scr * 0.65), 0),
    ...
    # 🚫 2. มุมซ้ายบน + ถนน (ถอย x เหลือ 0.35 ไม่ให้ทับแปลงผักแถวบน)
    cv2.rectangle(
        screenshot_bgr,
        (0, 0),
        (int(w_scr * 0.35), int(h_scr * 0.45)),
        (0, 0, 0),
        -1,
    )
...
def find_and_get_center(template_path, confidence=0.75):
    """ค้นหารูปภาพทั่วไป และเช็กข้ามไอคอนที่ Ignore"""
...
def safe_drag_to_target(start_pos, target_pos, duration=0.8):
    """ฟังก์ชันลากปลอดภัย: ลากจากจุด start ไปยัง target แบบระบุพิกัดชัดเจน ไม่หลุดจอ"""
    # 1. เลื่อนไปจุดเริ่มต้น (เช่น ไอคอนเคียว หรือ เมล็ดพันธุ์)
...
def plant_crops(soil_pos):
    """🌱 กระบวนการปลูกพืชแบบคุมพิกัดไม่ให้หลุดกรอบ"""
    print(f" 🌱 เริ่มกระบวนการปลูกพืชที่พิกัดดินว่าง {soil_pos}...")

    # คลิกแปลงดินเพื่อให้เมนูเมล็ดพันธุ์เด้งขึ้นมา
    pyautogui.click(soil_pos[0], soil_pos[1])
    time.sleep(0.6)

    # สแกนหาไอคอนเมล็ดพันธุ์
    seed_pos = find_and_get_center(IMAGE_SEED, confidence=0.60)

    if not seed_pos:
        # ถ้าหาไอคอนไม่เจอ ให้ใช้จุดเยื้องทางซ้ายบนของแปลงดินเล็กน้อย (จุดที่เมนูมักจะเด้ง)
        seed_pos = (soil_pos[0] - 60, soil_pos[1] - 40)
        print(f" ⚠️ ไม่พบภาพพืช -> ใช้พิกัดอ้างอิง: {seed_pos}")
    else:
        print(f" 🌾 พบภาพพืชที่พิกัด: {seed_pos}")

    print(
        f" 🚜 กำลังลากเมล็ดพันธุ์จาก {seed_pos} ไปยังแปลงผัก {soil_pos}..."
    )

    # ลากจากจุดเมล็ดพันธุ์ -> ไปปล่อยลงบนแปลงดินตรงๆ
    safe_drag_to_target(seed_pos, soil_pos, duration=0.6)
...
def harvest_and_plant_process():
    ...
    # 1. ตรวจหาพืชสุกจาก List
    ripe_pos = find_any_ripe_crop(confidence=0.68)

    if ripe_pos:
        print(f"✅ เจอแปลงพืชสุกที่พิกัด {ripe_pos}!")

        # คลิกต้นข้าวเพื่อให้ไอคอนเคียวเด้งขึ้นมา
        pyautogui.click(ripe_pos[0], ripe_pos[1])
        time.sleep(0.5)

        # 2. ค้นหาเคียวเกี่ยวข้าว
        sickle_pos = find_sickle(confidence=0.50)

        if sickle_pos:
            print(
                f" ⛏️ เจอเคียวที่ {sickle_pos}! กำลังลากเคียวไปเกี่ยวที่แปลงผัก..."
            )
            # ลากจากจุดเคียว -> ไปยังจุดแปลงผักตรงๆ
            safe_drag_to_target(sickle_pos, ripe_pos, duration=0.6)
        else:
            print(" ⚠️ ไม่เจอเคียว -> ใช้แผนสำรอง (Fallback Drag)")
            fallback_sickle = (ripe_pos[0] - 50, ripe_pos[1] - 30)
            safe_drag_to_target(fallback_sickle, ripe_pos, duration=0.6)
        ...
        print("❌ ไม่พบพืชสุก... กำลังสแกนหา 'แปลงดินว่าง'...")
...
if __name__ == "__main__":
    print("=" * 60)
    print(" 🤖 บอท Hay Day (ระบบ Multi-Crop/Sickle + Masking)")
